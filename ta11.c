...
/**
 * thandle_exit - Checks if the user entered the exit command
 * @buff: User's input
...
int thandle_exit(char *buff, char **cmds_list, char **commands)
{
	...
	{
		_write_history();
		_free_allocs(buff, cmds_list, commands, F_BUFF | F_CMDS);
		if (*t_process_exit_code() == 127)
			exit(2);
		exit(0);
	}

	status = tget_exit_status(commands[1]);
	...
	print_tbuiltin_error("exit: Illegal number: ", commands[1]);
	return (-1);
}

/**
 * tget_exit_status - Calculates the exit status as a number
 * @buff: User's input
 *
