...
/**
 * _handle_builtins - Executes the builtin funtions in case the command is one
...
int envars_builtins(char **commands)
{
	...
	if (_strcmp(commands[0], "setenv") == 0)
	{
		...
		t_setenv(commands[1], commands[2]);
		return (1);
	}
...
int other_set_buitlins(char **commands)
{
	...
	if (_strcmp(commands[0], "history") == 0)
	{
		if (commands[1] != NULL)
		{
			print_tbuiltin_error("history: No args allowed", "");
			return (1);
		}
