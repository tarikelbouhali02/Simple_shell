...
/**
 * t_parse_user_input - Buidls an array of strings as arguments
...
char **t_parse_user_input(char *str_input, char *delimiter)
{
	...
	if (str_input == NULL)
		tdispatch_error("Error while parsing the command\n");

	/* Count the number of arguments present in the input */
	args_count = t_count_args(str_input, delimiter);
	...
	{
		token = t_strtok(tkn_ptr, delimiter);
		if (token == NULL)
			break;
		tkn_ptr = NULL;
...
/**
 * t_count_args - Counts the number of arguments in a command string
...
int t_count_args(char *str_input, char *delimiter)
{
	...
	tkn_ptr = str_copy;
	while ((tkn = t_strtok(tkn_ptr, delimiter)) != NULL)
	{
		count++;
		tkn_ptr = NULL;
	}
...
/**
 * t_handle_PATH - Checks if the command to execute could be found in PATH's dirs
...
int t_handle_PATH(char **commands)
{
	...
	while (1)
	{
		tkn = t_strtok(tkn_ptr, ":");
		if (tkn == NULL)
			break;
		tkn_ptr = NULL;
		path = t_getpath(tkn, commands[0]);
...
/**
 * t_getpath - Creates a string representing a full path to file
...
{
	int dir_len =_tstrlen(dir);
	int filename_len =_tstrlen(filename);
	char *path;
	...
	t_strcpy(path, dir);
	t_strcat(path, "/");
	t_strncat(path, filename, filename_len + 1);
